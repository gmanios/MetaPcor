...
#     install.packages(new_cran_packages, dependencies = TRUE, quiet = TRUE)
#   }
#
...
#         install_github(paste0(owner, "/", repo), quiet = TRUE)
#       }
#     }
#   }
#
...
#
#   # Loading Libraries
#   libraries_to_load <- c("GEOquery", "corpcor", "space", "data.table",
#                          "GeneNet", "meta", "metafor", "dplyr",
#                          "tidyverse", "purrr", "DescTools", "ggplot2",
#                          "ff", "matrixcalc", "gprofiler2", "visNetwork",
#                          "geomnet", "igraph", "plotly", "readxl")
#
#   for(lib in libraries_to_load) {
#     if(!requireNamespace(lib, quietly = TRUE)) {
#       warning(paste0("Package ", lib, " not installed properly"))
#     } else {
...
# .GlobalEnv::install_and_load_packages()
...
# devtools::install_args(repos = "https://cran.r-project.org/")
...
load_GSE_data <- function(folder_path=NULL, GEO_names=NULL, GPL_list= NULL, target_namespace = NULL){
  if (!is.null(folder_path) & is.null(GEO_names)){
    ...
    list_of_files<-lapply(paste(folder_path, file_names,sep=''), readfiles)
    # print(list_of_files)
  }
...
pcor_neighborhood<-function(list_of_files, l1, l2=0, significant=TRUE, pvalue_thres=NULL, fdr_thres=NULL, coef_thres=NULL){
  ...
  for(i in 1:length(list_of_files)){
    pcor1<-space.neighbor(as.data.table(list_of_files[[i]]), lam1=l1, lam2=l2)$ParCor
