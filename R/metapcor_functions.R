...
DEG_meta <- function (folder_path, cases, controls){
  ...
  for (i in 1:length(list_of_studies)){
    ...
    # Read the file
    study <- read_excel(list_of_studies[i])
    study <- as.data.frame(study[1:10000,])
    ...
    # Make some convertions of the initial study 
    st <- lapply(study,as.numeric)
    st <- data.frame(st,row.names = study$ID)
    st$ID <-NULL
    
    # Convert study to data matrix and annotations to dataframe 
    st <- data.matrix(st)
    
    print(head(st))
    drops <- c("ID")
    
    annot = annot[ , !(names(annot) %in% drops)]
    annot = t(as.matrix(annot))
    
    an = data.frame(annot)
    
    colnames(an) = 'condition'
    #Append to the corresponding matrix
    expr_mat <- append(expr_mat,list(st))
    
    annotations_df <- append(annotations_df,list(an))
    
    col_names <- append(col_names, list_of_studies[i])
    
  }
  
  
  
  # Define the labels for cases and controls 
  pheno_Groups <- rep(c('condition'),times = length(list_of_studies))
  pheno_Cases <- as.list(rep(c(cases),times = length(list_of_studies)))
  pheno_Controls <- as.list(rep(c(controls),times =length(list_of_studies)))
  
  # Index everything list element so DExMA could be excecuted
  names(expr_mat) <- col_names
  names(pheno_Cases) <- col_names
  names(pheno_Controls ) <- col_names
  
  names(annotations_df) <- col_names
  
  # Initialize a new object for meta-analysis
  newObjectMA <- createObjectMA(listEX = expr_mat,
                                listPheno = annotations_df,
                                namePheno = pheno_Groups ,
                                
                                expGroups=pheno_Cases,
                                refGroups = pheno_Controls)
  
  # Call the DExMA meta-analysis function to find the DEGs
  resultsMA <- metaAnalysisDE(newObjectMA, typeMethod="REM",
                              missAllow=0.3, proportionData=0.5)
  
  # Find the DEGs where the standard p-values are < = FDR p-values
  list_of_DEGs = row.names(resultsMA[resultsMA$Pval<= resultsMA$FDR,])
  
  data_with_DEG = list ()
  
  for (i in 1:length(expr_mat)){
    
    cropped_df = expr_mat[[col_names[i]]] [row.names(expr_mat[[col_names[i]]] ) %in% list_of_DEGs,] 
    
    # Transpose and convert the matrix to data table 
    cropped_df = data.table(t(cropped_df))
    
    data_with_DEG = append(data_with_DEG, list(cropped_df))
  }
  
  
  
  # returns the DEGs that DExMA identified ([1]) cropped data.table of each study ([2])
  return (list(list_of_DEGs,data_with_DEG))
