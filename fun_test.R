...
load_GSE_data <- function(folder_path=NULL, GEO_names=NULL, GPL_list= NULL){   
  if (!is.null(folder_path) & is.null(GEO_names)){
    ...
    for(i in 1:length(list_of_files)){
      ...
      if(is.null(list_of_files[[i]][[paste(GEO_names[i],"_series_matrix.txt.gz", sep = "")]]@featureData@data[["Gene Symbol"]])==FALSE){
        ...
        new_cols <- gconvert(probe_list, organism = 'hsapiens', target= 'ENSG')
...
meta_pcor <- function(folder_path=NULL, GEO_names=NULL, GPL_list= NULL, my_list=NULL, option, method, meta_method= "random", pvalue_thres = NULL, fdr_thres = NULL, coef_thres = NULL,l1  = NULL ,l2 = NULL ){
  ...
  if (option == "Meta-anaysis and partia correlation meta-analysis" ){
  ...
  list_of_files<-load_GSE_data(folder_path, GEO_names, GPL_list)
...
DE_analysis <- function(list_of_studies,case,control, fold_threshold,p_value_threshold){
  ...
  #Sparse Correlation
  pcor_list <- pcor_neighborhood(list_of_files = expr_mat, l1 = 0.6, l2 = 0, significant=FALSE)   
